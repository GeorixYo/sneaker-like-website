...
function isThereDoubleIDs() {
    for(let itm1 in inventoryArr){
        for(let itm2 in inventoryArr){
            if (inventoryArr[itm1].id == inventoryArr[itm2].id && itm1 != itm2) return "there is double "+itm1+" "+itm2;
        }
    }
...
function amountPages(page=perPage) {
    ...
    else if (inventoryArr.length % page == 0) amountPages = inventoryArr.length / page;
    else amountPages = (inventoryArr.length - (inventoryArr.length % page)) / page + 1;
    return amountPages;
}
...
function cards(indexStart=currentPage, amount=perPage) {
    const cards = []
    let lastPage;
    if (indexStart * amount <= rightOrderArray.length) lastPage = indexStart * amount;
    ...
    for (let itm = (indexStart * amount - amount); itm < lastPage; itm++){
        ...
        function ifImg(){
            if(inventory.image) return `<img class="imga" draggable="false" src="${inventory.image}" alt="${inventory.name}">`;
            else return `<p class="card_name">Sorry, no image...</p>`;
        }
        //* returns correct Name of item position that fits in card 
        function thatName(){
            if (inventory.name.length > 27) return inventory.name.slice(0,27) + "...";
        ...
        function isButton(){
            if(!isItemInStock(rightOrdElem)) return `<p class="cost_dis">Not in stock</p><button class="cardbutton_dis" type="button">Add to card</button>`;
            else return `<p class="lowest_cost_size">for size ${lowestCostSize(rightOrdElem)}</p><p class="cost">$${isItemInStock(rightOrdElem)}</p><button class="cardbutton" type="button">Add to card</button>`;
        }
        ...
        function stockSizes() {
            ...
            for(let stock in inventory.inStock){
                let choosedSizeElem=""
                if (choosedSize[rightOrdElem] == stock) choosedSizeElem=" choosed_size_elem"
        ...
        let card = `
        <div class="inv_card${isItemInStock(rightOrdElem) ? "" : " no_in_stock"}" onclick="cardClickEvents(event)" id="${inventory.id}">
...
function cardPages(){
    ...
    if(totalPages==1 || currentPage==1) pagesQube = `<div class="pages back_page_dis"><p><</p></div>`;
    else pagesQube = `<div class="pages  back_page" ${pageFunc}><p><</p></div>`;

    for (let i = 1; i <= totalPages;i++){
        if(currentPage==i) pagesQube += `<div class="pages current_page" id="current${i}"><p>${i}</p></div>`;
        else pagesQube += `<div class="pages other_page" ${pageFunc} id="current${i}"><p>${i}</p></div>`;
    }

    if (totalPages==1 || currentPage==totalPages) pagesQube += `<div class="pages forward_page_dis"><p>></p></div>`;
    else pagesQube += `<div class="pages forward_page" ${pageFunc}><p>></p></div>`;
    return pagesQube;
}
...
function changeCardPage(event){
    if (event.target.closest(".other_page")) currentPage = parseInt(`${event.target.closest(".other_page").id}`.slice(7));
...
function dynamicCost(event, mouse = true){
    const selector = document.getElementById(event.target.closest(".inv_card").id).querySelector(".bottom_sec>.lowest_cost_size"), selector2=document.getElementById(event.target.closest(".inv_card").id).querySelector(".bottom_sec>.cost")
    const numb = mouse ? event.target.id : event.target.querySelector(".is_stock").id
    if (mouse) {
        selector.textContent = "for size " + parseFloat(numb);
        selector2.textContent = "$" + inventoryArr[parseInt(`${numb}`.slice(`${parseFloat(numb)}`.length+3))].inStock[parseFloat(numb)];
    } else {
        selector.textContent = "for size " + choosedSize[parseInt(`${numb}`.slice(`${parseFloat(numb)}`.length+3))];
        selector2.textContent = "$" + inventoryArr[parseInt(`${numb}`.slice(`${parseFloat(numb)}`.length+3))].inStock[choosedSize[parseInt(`${numb}`.slice(`${parseFloat(numb)}`.length+3))]]
    }
}
...
function like(event, Curpage = "card"){
    if (Curpage == "card") {
        ...
        let targetElem
        if (event.target == document.getElementById(`${closest}`).querySelector(`.heart>p`)) targetElem = event.target
        ...
        console.log(closest)
        let targetElem
        if (event.target == document.querySelector(`.preview_heart>p`)) targetElem = event.target
        else targetElem = event.target.querySelector("p")
        if (wishlist.includes(closest)) {
            wishlist.splice(wishlist.indexOf(closest), 1);
            return targetElem.innerHTML = '🤍';
...
function previewPage(event){
    ...
    function availableSizes(event){
        ...
        if(!isItemInStock(thatArr)) return '<p class="preview_sizes_not">No available sizes</p>';
        let stockPositions = ``;
        for(let stock in thatArrInStock){
            if (thatArrInStock[stock]) { 
                stockPositions += `<div class="${choosedSize[thatArr] == stock ? 'preview_that_size_active' : 'preview_that_size'}" id="${stock}preview${thatArr}"><p class="preview_size_size">${stock}</p><p class="preview_size_cost">$${thatArrInStock[stock]}</p></div>`;
            }
        }
        return stockPositions;
    }
    function isDescription(event){
        ...
        for (let i in inv) {
            ...
            if(inv[i]) description += `<p class="inner_description_${i}">${a}: ${inv[i]}</p>`
        }
        return description;
    }
    ...
    <div class="whole_preview" id="preview_${event.target.closest(".inv_card").id}_${returnThatArray(event)}">  
        <div class="preview_page">
            ...
            <div class="preview_imagegalery">
                ...
                <div class="preview_imagegalery">
                    <img class="inner_galery_image" id="previewimg_0" src="${galeryImage(event)}" alt="">
                </div>
            </div>
            <div class="preview_name_sizes">
                <div class="inner_preview_name">
                    <p class="preview_inner_name">${inventoryArr[returnThatArray(event)].name}</p>
                    <p class="preview_inner_id">id: ${inventoryArr[returnThatArray(event)].id}</p>
                </div>
                <div class="inner_preview_sizes">
                    <p class="preview_inner_sizes">Available Sizes:</p>
                    <div class="inner_model_sizes">${availableSizes(event)}</div>
                </div>
            </div>
            ...
            <div class="preview_cost_and_buy">
                <p class="preview_inner_cost">${choosedSize[thatArray] ? "$"+inventoryArr[thatArray].inStock[choosedSize[thatArray]] : "No sizes"}</p>
                ${isItemInStock(thatArray)? '<p class="preview_inner_cost_sizesign">for ' + choosedSize[thatArray] + ' size</p>' : "" }
...
function changeSizePrice(event){ 
    document.querySelector('.preview_that_size_active').className = 'preview_that_size';
    const arr = parseInt(event.target.id.slice(`${parseFloat(event.target.id)}`.length+7)) 
    const sizeArr = event.target.id.slice(0, `${parseFloat(event.target.id)}`.length);
    event.target.className = 'preview_that_size_active'
    document.querySelector('.preview_inner_cost').textContent = `$${inventoryArr[arr].inStock[sizeArr]}`
    document.querySelector('.preview_inner_cost_sizesign').textContent = `for ${sizeArr} size`
...
function cardClickEvents(event) {
    ...
    else if (event.target.className == "heart" || event.target == document.getElementById(`${maincard}`).querySelector(`.heart>p`)) like(event);
    else previewPage(event)
}
function previewClickEvents(event){
    if(event.target.className == "exit_preview_screen" || event.target.className == "exit_preview_cross" || event.target == document.querySelector(".exit_preview_cross>p")) {
        document.querySelector(".preview_page").innerHTML = ``
        document.querySelector("section").style.cssText = 'display:none;'
    } else if (event.target.className == "preview_heart" || event.target == document.querySelector(".preview_heart>p")) like(event, "preview")
    else if (event.target.className == "preview_that_size" ) changeSizePrice(event);
}
